...
FILE_ID=1N3MbvpgZAmNgiZgnpXAQFzHrU7Tt3Blb
OUTPUT_GZ=arxiv_cs_abstracts.txt.gz
...
if [ -n "${CONFIRM_TOKEN}" ]; then
  ...
  wget --load-cookies "${COOKIE_JAR}" "${DOWNLOAD_URL}" -O "${OUTPUT_GZ}"
fi

rm -f "${COOKIE_JAR}" "${TMP_HTML}"

if ! gzip -t "${OUTPUT_GZ}" >/dev/null 2>&1; then
